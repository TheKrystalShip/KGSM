...
- Standalone Instances 2.0
    - Each instance now manages its own downloading, updating, and backups via a dedicated `<instance>.manage.sh` script, reducing KGSM’s responsibilities to the initial setup.
    - A symbolic link to `<instance>.manage.sh` will be provided in a system `$PATH` directory, enabling system-wide execution.

- Container Images 1.0
    - Support for container-based instances, expanding the deployment options.
    - A curated registry of KGSM-compatible container images on DockerHub.
    - Feature parity maintained with existing native instances.

- UPnP Support
    - Automatic port forwarding configuration using UPnP, eliminating manual setup hassles.
    - Leverages existing port definitions to open network ports via UPnP.
    - UPnP functionality can be enabled or disabled individually for each instance.

- Improved Portability
    - Change the shebang from `#!/bin/bash` to `#!/usr/bin/env bash` to enhance compatibility across environments.
