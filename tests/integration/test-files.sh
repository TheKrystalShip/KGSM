...
instance_name=$(./modules/instances.sh --create factorio.bp --install-dir "$install_dir" --name "file-test-instance")
...
file_create_result=$(./modules/files.sh -i "$instance_name" --create)
...
file_remove_result=$(./modules/files.sh -i "$instance_name" --remove)
